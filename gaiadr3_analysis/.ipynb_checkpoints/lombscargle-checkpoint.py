...
def period(t: pd.DataFrame = None, mag: pd.DataFrame = None, frequency_range: list[float] = None, jd: bool=True):
    ...
    #Frequency Range
    # If user didn't specify freq_range, set it automatically with the Nyquist frequency.
...
#Add option for period range instead of freq
def plot(t: pd.DataFrame = None, mag: pd.DataFrame = None, period_range: list[float] = None, lims: list[float] = None, jd: bool=True):
    #Default example for if t & y are not inputted.
    if t is None or mag is None:
        rand = np.random.default_rng(42)
        #Time
